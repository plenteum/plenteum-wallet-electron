...
const WALLET_VERSION = app.getVersion() || '0.3.x';
...
const DEFAULT_SETTINGS = {
    ...
    pubnodes_custom: ['127.0.0.1:44016'],
...
const WIN_TITLE = `${config.appName} ${WALLET_VERSION} - ${config.appDescription}`;
...
log.info(`Starting PlenteumWallet ${WALLET_VERSION}`);
...
function createWindow() {
    ...
    let bgColor = darkmode ? '#000000' : '#0076A8';

    const winOpts = {
        title: WIN_TITLE,
        icon: path.join(__dirname, 'src/assets/plenteumwallet_icon.png'),
        frame: true,
...
function initSettings() {
    ...
    settings.set('version', WALLET_VERSION);
    serviceBinCheck();
