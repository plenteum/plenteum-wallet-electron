...
const SESSION_KEY = 'plewall';

var PlenteumWalletSession = function (opts) {
    if (!(this instanceof PlenteumWalletSession)) return new PlenteumWalletSession(opts);
    opts = opts || {};
    ...
    this.sessDefault = {
        ...
        servicePath: settings.get('service_bin', 'wallet-service'),
        configUpdated: false,
...
PlenteumWalletSession.prototype.get = function (key) {
    key = key || false;
    if (!key) {
...
PlenteumWalletSession.prototype.getDefault = function (key) {
    if (!key) {
...
PlenteumWalletSession.prototype.set = function (key, val) {
    if (!this.keys.includes(key)) {
        throw new Error(`Invalid session key: ${key}`);
    }
...
PlenteumWalletSession.prototype.reset = function (key) {
    if (key) {
...
PlenteumWalletSession.prototype.destroy = function () {
    return sessionStorage.removeItem(this.sessKey);
};

module.exports = PlenteumWalletSession;