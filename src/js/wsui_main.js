...
const PlenteumWalletAddressbook = require('./ws_addressbook');
...
let addressBook = new PlenteumWalletAddressbook(ADDRESS_BOOK_DEFAULT_PATH);
...
function showInitialPage() {
    ...
    let versionInfo = document.getElementById('plenteumWalletVersion');
    if (versionInfo) versionInfo.innerHTML = WS_VERSION;
    let tsVersionInfo = document.getElementById('walletServiceVersion');
    ...
    function loadAddressBook(params) {
        ...
        if (params) {
            ...
            if (params.name === 'default') {
                addressBook = new PlenteumWalletAddressbook(ADDRESS_BOOK_DEFAULT_PATH);
            } else {
                addressBook = new PlenteumWalletAddressbook(params.path, params.name, params.pass);
            }
        }
    ...
    overviewButtonCreate.addEventListener('click', () => {
        ...
        wsutil.validateWalletPath(filePathValue, DEFAULT_WALLET_PATH).then((finalPath) => {
            ...
            wsmanager.createWallet(
                ...
                wsutil.showToast('Your wallet has been created, you can now open your wallet!', 12000);
            }).catch((err) => {
    ...
    importKeyButtonImport.addEventListener('click', () => {
        ...
        wsutil.validateWalletPath(filePathValue, DEFAULT_WALLET_PATH).then((finalPath) => {
            ...
            wsmanager.importFromKeys(
                ...
                walletOpenInputPath.value = walletFile;
                changeSection('section-overview-load');
                wsutil.showToast('Your wallet has been imported, you can now open your wallet!', 12000);
            }).catch((err) => {
